...
| geom | Adds a field, namedgeom, to each event. This field contains geographic data structures for polygon geometry in JSON and is used for the choropleth map visualization. | geomfilter |
...
| head | Returns the first numbernof specified results. | reverse, tail |
...
| mpreview | Returns a preview of the rawmetric data pointsin a specified metric index that match a provided filter. | mcatalog,mstats,msearch |
| msearch | Alias for thempreviewcommand. | mcatalog,mstats,mpreview |
...
| multisearch | Run multiplestreaming searchesat the same time. | append, join |
...
| outputtext | Outputs the raw text field (_raw) of results into the_xmlfield. | outputcsv |
...
| timewrap | Displays, or wraps, the output of thetimechart commandso that everytimewrap-spanrange of time is a different series. | timechart |
...
| typelearner | Deprecated. Usefindtypesinstead. Generates suggested eventtypes. | typer |
